...
In this guide, we will utilize [signature-based minting](https://docs.web3sdks.com/advanced-features/on-demand-minting) of NFTs as a mechanism to reward users of a specific community. We connect user's with their Discord account, and generate signatures for an NFT if the user is a **member** of the Discord server.

**Check out the Demo here**: https://community-rewards.web3sdks-template.com

If you're interested in reading the basics of signature-based minting, we recommend starting with this example repository: https://github.com/web3sdks-template/signature-based-minting-next-ts

## Tools:

- [**React SDK**](https://docs.web3sdks.com/react): To connect to our NFT Collection Smart contract via React hooks such as [useNFTCollection](https://docs.web3sdks.com/react/react.usenftcollection), and allow users to sign in with [useMetamask](https://docs.web3sdks.com/react/react.usemetamask).

- [**NFT Collection**](https://docs.web3sdks.com/pre-built-contracts/nft-collection): This is the smart contract that our NFTs will be created into.

- [**TypeScript SDK**](https://docs.web3sdks.com/typescript): To mint new NFTs with [signature based minting](https://docs.web3sdks.com/typescript/sdk.nftcollection.signature)!
...
- Create an NFT Collection contract via the web3sdks dashboard on the Polygon Mumbai (MATIC) test network.

- Create a project using this example by running:

```bash
npx web3sdks create --template community-rewards
```
...
- We use the web3sdks discord server ID `834227967404146718`. Find and replace instances of this ID with your own Discord server ID. You can learn how to get your Discord server ID from [this guide](https://www.alphr.com/discord-find-server-id/).
...
In this example, we use signature-based minting to exclusively grant signatures to users who are members of the Discord server with ID `834227967404146718`; the web3sdks discord server.
...
For the MetaMask connection, we are using the [useMetamask](https://docs.web3sdks.com/react/react.usemetamask) hook from the web3sdks React SDK.

```jsx
const connectWithMetamask = useMetamask();
```

This works because we have the `Web3sdksProvider` setup in our [_app.js](/pages/_app.js) file, which allows us to use all of the web3sdks React SDK's helpful hooks.
...
res.status(200).json({ web3sdksMembership: web3sdksDiscordMembership });
```
...
data ? (
  ...
  <div>
    ...
    <a href={`https://discord.com/invite/web3sdks`}>Join Server</a>
  </div>
);
```
...
Firstly, we initialize the web3sdks SDK using our private key.
...
const signedPayload = await nftCollection.signature.generate({
  to: claimerAddress,
  metadata: {
    name: `${session.user.name}'s Web3sdks Discord Member NFT`,
    image: `${session.user.image}`,
    description: `An NFT rewarded to ${session.user.name} for being a part of the web3sdks community!`,
  },
});
```
